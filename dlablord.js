    ...
    (err, res) => {
      ...
      if (events.length) {
        ...
        events.map((event, i) => {
          ...
          rake(event.summary, "./stopwords.txt").then(function (keywords) {
            console.log(keywords)
            const keyword = toTitleCase(Object.keys(keywords)[0])
            // console.log(`[${keyword}]`)

            setChannelName(keyword)
          })
        })
      } else {
...
client.on("ready", () => {
  ...
  fs.readFile("credentials.json", (err, content) => {
    ...
    const performCalendar = function() {
        authorize(JSON.parse(content), listEvents);
    }

    performCalendar();
    setInterval(performCalendar, 1000 * 60 * (process.env.REFRESH_INTERVAL || 30))
  });
})
...
function setChannelName(name) {
  const channel = client.channels
    ...
    .then(function (channel) {
      // console.log(channel);
      channel.setName(toTitleName(name))
    })
    .catch(console.error)
}
